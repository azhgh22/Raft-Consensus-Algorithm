...
func TestFailAgree3B(t *testing.T) {
	...
	// fmt.Printf("Server %d disconnected from others\n", (leader+1)%servers)
	tester.AnnotateConnection(ts.g.GetConnected())
	...
	// fmt.Printf("Server %d recconected\n", (leader+1)%servers)
...
func TestRejoin3B(t *testing.T) {
	...
	leader1 := ts.checkOneLeader()
	ts.g.DisconnectAll(leader1)
	// fmt.Printf("Server %d disconected\n", leader1)
	tester.AnnotateConnection(ts.g.GetConnected())
...
func TestFigure8Unreliable3C(t *testing.T) {
	...
	for i := 0; i < servers; i++ {
		if !ts.g.IsConnected(i) {
			ts.g.ConnectOne(i)
			// fmt.Printf("server %d is recconected\n", i)
		}
	}
	tester.AnnotateConnection(ts.g.GetConnected())
